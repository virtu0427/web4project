...
export default function Home() {
  ...
  const fetchTrendingMovies = async (pageNum: number, append: boolean = false) => {
    try {
      setIsLoading(true);
      setError(null);
  ...
  useEffect(() => {
    if (isInfiniteScroll) {
      ...
      fetchTrendingMovies(page, false);
    }
    ...
    <div className="min-h-screen bg-gray-900 py-4">
      <div className="max-w-7xl mx-auto px-2 sm:px-4">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold text-gray-100">이번 주 인기 영화</h1>
        ...
        <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-4">
          {trendingMovies.map((movie, index) => (
            <div
              key={`${movie.id}-${index}`}
              ref={
                isInfiniteScroll && index === trendingMovies.length - 1
                  ? lastMovieElementRef
                  : null
              }
            >
